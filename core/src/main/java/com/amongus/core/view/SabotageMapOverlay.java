...
public class SabotageMapOverlay {
    ...
    private MapType currentMapType = MapType.MAPA_1;

    // ── Botones de sabotaje ───────────────────────────────────
    // Posiciones relativas al panel (fracción del panel)
    private float[][] sabotagePositionsMap1 = {
        {2203f / 3840f, 1740f / 2160f}, // Internet (Librería)
        {763f / 3840f,  107f / 2160f}   // Luces (Cables/Eléctrico)
    };

    private float[][] sabotagePositionsMap2 = {
        {1560f / 3840f, 510f / 2160f},  // Internet (Etapa en Proyecto)
        {179f / 3840f,  1420f / 2160f}  // Luces (Cables)
    };

    // Modifica el metodo recalculateSabotageButtons para que use el mapa correcto
    private void recalculateSabotageButtons(MapType currentMap) {
        float[][] activePositions = (currentMap == MapType.MAPA_1)
            ? sabotagePositionsMap1
            : sabotagePositionsMap2;

        for (int i = 0; i < activePositions.length; i++) {
            float bx = panelX + panelW * activePositions[i][0] - ALERT_BTN_SIZE / 2f;
            // IMPORTANTE: En el overlay de Sebastián, el eje Y suele estar invertido respecto al renderizado
            // Si ves que aparecen "espejeados" verticalmente, cambia el cálculo a: 1.0f - activePositions[i][1]
            float by = panelY + panelH * activePositions[i][1] - ALERT_BTN_SIZE / 2f;
    ...
    public void setMap(MapType mapType) {
        String mapPath = mapType.getVisualPath();
    ...
    public void render(SpriteBatch batch, ShapeRenderer shapeRenderer,
        ...
        // ── Botones de sabotaje ───────────────────────────────
        recalculateSabotageButtons(currentMapType);
